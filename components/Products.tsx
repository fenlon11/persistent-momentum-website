...
const statusConfig: Record<string, { label: string; dotClass: string; borderClass: string; bgClass: string }> = {
  ...
  'coming-soon': {
    ...
    borderClass: 'border-slate-700/30 hover:border-[#1E5BFF]/50',
    bgClass: 'from-[#1E5BFF]/5 to-transparent',
  },
  'active': {
    label: 'Live',
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1E5BFF]/10 border border-[#1E5BFF]/20 backdrop-blur-sm mb-6">
            <span className="text-sm text-[#1E5BFF] font-medium">Products</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
            What we
            <span className="text-[#1E5BFF]"> ship</span>
          </h2>
                ...
                <div className={`relative h-full bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl border ${config.borderClass} p-6 transition-all duration-500 overflow-hidden hover:scale-[1.02]`}>
                  ...
                  <div className="relative space-y-4">
                    ...
                    <div>
                      ...
                      {product.isPrimary && (
                        <span className="text-xs text-[#1E5BFF] font-medium">Featured</span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2">
                      {product.features.slice(0, 4).map((feature) => (
                        <div key={feature.title} className="flex items-start gap-1.5 text-slate-300">
                          <svg className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-[#1E5BFF]" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="flex items-center gap-2 pt-2 text-[#1E5BFF] text-sm font-medium group-hover:gap-3 transition-all">
                      Learn More
