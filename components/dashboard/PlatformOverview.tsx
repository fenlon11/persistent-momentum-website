...
const CATEGORY_COLORS: Record<string, string> = {
  preference: 'bg-[var(--color-primary)]',
  fact: 'bg-emerald-500',
  ...
  return (
    <div className="space-y-6">
      ...
      {memory && Object.keys(memory.byCategory).length > 0 && (
        <div className="bg-[var(--color-bg-light-secondary)] border border-[var(--color-border-light)] rounded-xl p-4">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {skills?.invocations.slice(0, 10).map((inv) => (
              <div key={inv.id} className="flex items-center gap-2 text-xs">
                ...
                {inv.model_used && (
                  <span className="text-[var(--color-text-dark-muted)]">{inv.model_used}</span>
                )}
                {inv.duration_ms && (
                  <span className="text-[color:rgba(100,116,139,0.7)]">{inv.duration_ms}ms</span>
                )}
              </div>
            ))}
            {!skills?.invocations.length && (
              <p className="text-[var(--color-text-dark-muted)] text-xs">
            ...
            {sessions?.logs.slice(0, 10).map((log) => (
              <div key={log.id} className="flex items-start gap-2 text-xs">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[color:rgba(100,116,139,0.7)] ml-2">{log.log_date}</span>
                </div>
              </div>
            ))}
            {!sessions?.logs.length && (
              <p className="text-[var(--color-text-dark-muted)] text-xs">No session logs recorded yet</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {memory.recent.map((mem) => (
              <div key={mem.id} className="flex items-start gap-2 text-xs">
                ...
                />
                <span className="text-[var(--color-text-dark)] flex-1 line-clamp-2">
                  {mem.content}
                </span>
              </div>
            ))}
        ...
        <div className="bg-[var(--color-bg-light-secondary)] border border-[var(--color-border-light)] rounded-xl p-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-[var(--color-text-dark-muted)] border-b border-[var(--color-border)]">
              ...
              <tbody>
                {skills.stats.slice(0, 10).map((stat) => (
                  <tr key={stat.skill_name} className="border-b border-[var(--color-border-light)]">
                    <td className="py-2 text-white font-medium">
                      {stat.skill_name}
                    </td>
                    <td className="py-2 text-right text-[var(--color-text-dark)]">
                      {stat.total_count}
                    </td>
                    ...
                    <td className="py-2 text-right text-[var(--color-text-dark-muted)]">
...
function StatCard({
  ...
  const colorClasses: Record<string, string> = {
    ...
    blue: 'border-[rgba(21,68,142,0.25)] text-[var(--color-primary)]',
    purple: 'border-purple-500/20 text-purple-400',
    red: 'border-red-500/20 text-red-400',
    slate: 'border-[var(--color-border-light)] text-[var(--color-text-dark-muted)]',
  };

  return (
    <div
      className={`bg-[var(--color-bg-light-secondary)] border ${colorClasses[color]} rounded-xl p-3 text-center`}
    >
      <div className={`text-2xl font-bold ${colorClasses[color]}`}>{value}</div>
      <div className="text-xs text-[var(--color-text-dark-muted)]">{label}</div>
    </div>
  );
}
